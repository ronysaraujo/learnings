...
const {gets, print, geraEntradas} = require('./funcoes-auxiliares.js');

geraEntradas([3000, 250]);

function buscarAliquota(salario) {
...
function calcularSalario(salarioBruto, imposto, adicional){
    return (salarioBruto - imposto + adicional).toFixed(2);
}

let salarioBruto = gets();
let aliquota = buscarAliquota(salarioBruto);
let adicional = gets();
let imposto = salarioBruto*aliquota;

let salarioLiquido = calcularSalario(salarioBruto, imposto, adicional);
